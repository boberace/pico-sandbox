...
void drawGPixel(short x, short y, char color) {
    // Range checks (640x480 display)
    //if (x > 520) x = 520 ;
   // if (x < 0) x = 0 ;
    //if (y < 0) y = 0 ;
    //if (y > 479) y = 479 ;
    if((x > 519) | (x < 121) | (y > 449) | (y < 51) ) return;
...
void drawHLine(int x, int y, int w, char color) {
  ...
  // draw rest of line
  int len = (w>>1) + 1 ;
  if (len>0 && y<470 ) memset(&vga_data_array[320*y+(x>>1)], color, len) ;
 // original code
    // for (int i=x; i<=(x+w); i++) {
    //     drawPixel(i, y, color) ;
    // }
...
void clear_frame(short x1, short y1, short x2, short y2) {
  for(int i=y1; i<y2; i++){
    memset(&vga_data_array[320*i+(x1>>1)], 0, (x2-x1)>>1) ;
  };
}

void clear_frame2() {
    memset(&vga_data_array[320*50], 0, (320*429) );
}
