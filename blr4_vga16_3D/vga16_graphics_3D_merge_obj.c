...
// ===== build_Tpersp()
// for now just use a test perspective matrix
// =================
...
// =========== merge two objects
  ...
  void merge_object(object* o1, object* o2, object* o3){
...
static PT_THREAD (protothread_graphics(struct pt *pt)) {
   ...
   //writeStringBold("GLCD BOLD Font Test") ;
    ...
    static matrix TrotZ, Ttrans, TrotY, Ttrans_tetra, Ttrans_merge ;
    static int t;
    ...
    merge_object(&cube, &tetrahedron, &test_merge); 
    ...
    build_Translate (float_to_s15x16(-2.0), float_to_s15x16(-2), float_to_s15x16(2), &Ttrans_merge);

    // === define the camera ===
    vector from = {float_to_s15x16(1.0), float_to_s15x16(-0.0), float_to_s15x16(0.0)} ;
    vector to = {0,0,0} ;
    vector approxUp = {0,one,one};
    //
    ...
    while(true) {
        ...
        for (t=0; t<1000; t++){
          ...
          view_insert(tetrahedron.N_vertex, tetrahedron.N_face, tetra_model_vertex, tetrahedron.face) ;
          view_insert(cube.N_vertex, cube.N_face, cube_model_vertex, cube.face) ;
          view_insert(test_merge.N_vertex, test_merge.N_face, test_merge_model_vertex, test_merge.face) ;
          ...
          // depth sort
          int I[view.N_vertex] ;
          for(int i=0; i<view.N_vertex; i++) I[i] = i ;
          QuickSort(vz, I, 0, view.N_vertex-1) ;
          //for(int i=0; i<view.N_vertex; i++) printf("%d\n\r", I[i]);
