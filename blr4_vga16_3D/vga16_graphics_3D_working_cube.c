...
cube cube1 ={
  8, 12,
  { 0,0,0, one,0,0, one,one,0, 0,one,0, 0,0,one, one,0,one, one,one,one, 0,one,one},
...
static PT_THREAD (protothread_graphics(struct pt *pt)) {
   ...
   //writeStringBold("GLCD BOLD Font Test") ;
    ...
    static vector view_vertex[8], tetra_rot_vertex[4], temp1, temp2, temp3 ;
    static vector  cube_rot_vertex[8], cube_trans_vertex[8], cube_xrot_vertex[8];
    static matrix TrotZ, Ttrans, TrotY ;
    static int t;
    ...
    while(true) {
        ...
        for (t=0; t<1000; t++){
          ...
          // memset to clear frame fbuffer region
          clear_frame(50, 250) ;
          for (int i=0; i<8; i++){
            VxM(&cube1.vertex[i], &Ttrans, &cube_trans_vertex[i]); //
           // VxM(&cube1.vertex[i], &TrotZ, &cube_rot_vertex[i]); //
            VxM(&cube_trans_vertex[i], &TrotZ, &cube_rot_vertex[i]); //
            VxM(&cube_rot_vertex[i], &TrotY, &cube_xrot_vertex[i]); //
            //
            VxM(&cube_xrot_vertex[i], &Tcamera, &view_vertex[i]); //&Tcamera
          ...
          for (int i=0; i<12; i++){
            int x1, y1, x2, y2 ;
            Vsub(&view_vertex[cube1.face[i][0]], &view_vertex[cube1.face[i][2]], &temp1) ;
            Vsub(&view_vertex[cube1.face[i][0]], &view_vertex[cube1.face[i][1]], &temp2) ;
            Vcross(&temp1, &temp2, &temp3) ;
            if (Vdot(&temp3, &Nview) < 0){   //Vdot(&temp3, &Nview)<0
              //
              x1 = s15x16_to_int(view_vertex[cube1.face[i][0]].x) ;
              y1 = s15x16_to_int(view_vertex[cube1.face[i][0]].y) ;
              x2 = s15x16_to_int(view_vertex[cube1.face[i][1]].x) ;
              y2 = s15x16_to_int(view_vertex[cube1.face[i][1]].y) ;
              drawLine(x1, y1, x2, y2, WHITE ) ;
            // printf("%d %d %d %d\n\r", x1, y1, x2 ,y2) ;

              x1 = s15x16_to_int(view_vertex[cube1.face[i][2]].x) ;
              y1 = s15x16_to_int(view_vertex[cube1.face[i][2]].y) ;
              x2 = s15x16_to_int(view_vertex[cube1.face[i][1]].x) ;
              y2 = s15x16_to_int(view_vertex[cube1.face[i][1]].y) ;
             drawLine(x1, y1, x2, y2, WHITE ) ;
              //printf("%d %d %d %d\n\r", x1, y1, x2 ,y2) ;
              ...
              x1 = s15x16_to_int(view_vertex[cube1.face[i][0]].x) ;
              y1 = s15x16_to_int(view_vertex[cube1.face[i][0]].y) ;
              x2 = s15x16_to_int(view_vertex[cube1.face[i][2]].x) ;
              y2 = s15x16_to_int(view_vertex[cube1.face[i][2]].y) ;
              drawLine(x1, y1, x2, y2, WHITE ) ;
              //printf("%d %d %d %d\n\r", x1, y1, x2 ,y2) ;
            }
          }
