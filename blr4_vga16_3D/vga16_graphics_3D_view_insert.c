   ...
   //writeStringBold("GLCD BOLD Font Test") ;
    ...
    while(true) {
        ...
        for (t=0; t<1000; t++){
          ...
          // cube modeling
          for (int i=0; i<cube1.N_vertex; i++){
           ...
           // VxM(&cube1.vertex[i], &TrotZ, &cube_rot_vertex[i]); //
            ...
            //
            VxM(&cube_model_vertex[i], &Tcamera, &cube_view_vertex[i]); //&Tcamera
          }

          // etrahedron modeling
          for (int i=0; i<tetra1.N_vertex; i++){
            VxM(&tetra1.vertex[i], &TrotY, &tetra_model_vertex[i]);
            VxM(&tetra_model_vertex[i], &Tcamera, &tetra_view_vertex[i]) ;
          }

         //view_insert(int Nv, int Nf, vector v[], face f[])
         view_insert(tetra1.N_vertex, tetra1.N_face, tetra_view_vertex, tetra1.face) ;

         view_insert(cube1.N_vertex, cube1.N_face, cube_view_vertex, cube1.face) ;
         //view_insert(tetra1.N_vertex, tetra1.N_face, tetra_view_vertex, tetra1.face) ;
