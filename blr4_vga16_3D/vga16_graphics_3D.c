...
void view_insert(int Nv, int Nf, vector v[], face f[]){
  ...
  int j = 0;
  for (int i=current_Nv; i<Nv; i++){
    view.vertex[i].x = v[j].x ;
    view.vertex[i].y = v[j].y ;
    view.vertex[i].z = v[j].z ;
  }
  j= 0 ;
  for (int i=current_Nf; i<Nf; i++){
    view.face[i].v0 = f[j].v0 ;
    view.face[i].v1 = f[j].v1 ;
    view.face[i].v2 = f[j].v2 ;
  }
  view.N_vertex += Nv;
  view.N_face += Nf ;
}
...
static PT_THREAD (protothread_graphics(struct pt *pt)) {
   ...
   //writeStringBold("GLCD BOLD Font Test") ;
    ...
    static vector view_vertex[8], tetra_rot_vertex[4] ;
    static vector temp1, temp2, temp3 ;
    static vector  cube_model_vertex[8] ; 
    ...
    while(true) {
        ...
        for (t=0; t<1000; t++){
          ...
          // memset to clear frame buffer region y1 to y2
          
          for (int i=0; i<8; i++){
            VxM(&cube1.vertex[i], &Ttrans, &cube_model_vertex[i]); //
           // VxM(&cube1.vertex[i], &TrotZ, &cube_rot_vertex[i]); //
            ...
            //
            VxM(&cube_model_vertex[i], &Tcamera, &view_vertex[i]); //&Tcamera
          ...
          for (int i=0; i<12; i++){
            int x1, y1, x2, y2 ;
            Vsub(&view_vertex[cube1.face[i].v0], &view_vertex[cube1.face[i].v2], &temp1) ;
            Vsub(&view_vertex[cube1.face[i].v0], &view_vertex[cube1.face[i].v1], &temp2) ;
            Vcross(&temp1, &temp2, &temp3) ;
            if (Vdot(&temp3, &Nview) < 0){   //Vdot(&temp3, &Nview)<0
              //
              x1 = s15x16_to_int(view_vertex[cube1.face[i].v0].x) ;
              y1 = s15x16_to_int(view_vertex[cube1.face[i].v0].y) ;
              x2 = s15x16_to_int(view_vertex[cube1.face[i].v1].x) ;
              y2 = s15x16_to_int(view_vertex[cube1.face[i].v1].y) ;
              drawLine(x1, y1, x2, y2, WHITE ) ;
            // printf("%d %d %d %d\n\r", x1, y1, x2 ,y2) ;

              x1 = s15x16_to_int(view_vertex[cube1.face[i].v2].x) ;
              y1 = s15x16_to_int(view_vertex[cube1.face[i].v2].y) ;
              x2 = s15x16_to_int(view_vertex[cube1.face[i].v1].x) ;
              y2 = s15x16_to_int(view_vertex[cube1.face[i].v1].y) ;
             drawLine(x1, y1, x2, y2, WHITE ) ;
              //printf("%d %d %d %d\n\r", x1, y1, x2 ,y2) ;
              ...
              x1 = s15x16_to_int(view_vertex[cube1.face[i].v0].x) ;
              y1 = s15x16_to_int(view_vertex[cube1.face[i].v0].y) ;
              x2 = s15x16_to_int(view_vertex[cube1.face[i].v2].x) ;
              y2 = s15x16_to_int(view_vertex[cube1.face[i].v2].y) ;
              drawLine(x1, y1, x2, y2, WHITE ) ;
              //printf("%d %d %d %d\n\r", x1, y1, x2 ,y2) ;
            }
          }
